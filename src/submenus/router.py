...
async def list_menu_submenu(menu_id: str, db: Session=Depends(get_db)):
...
async def get_submenu(submenu_id: str, response: Response, db: Session=Depends(get_db)):
...
async def post_menu_submenu(menu_id: str, submenu: schemas.SubmenuCreate, db: Session=Depends(get_db)):
...
async def patch_submenu(submenu_id: str, submenu: schemas.SubmenuUpdate, db: Session=Depends(get_db)):
