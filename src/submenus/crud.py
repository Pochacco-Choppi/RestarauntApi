...
def get_submenu(db: Session, submenu_id: UUID4):
    db_submenu = db.query(Submenu).filter(Submenu.id == submenu_id).first()
    return db_submenu

def list_menu_submenu(menu_id: UUID4, db: Session, skip: int = 0, limit: int = 100):
    db_submenus = db.query(Submenu).filter(Submenu.menu_id == menu_id).offset(skip).limit(limit).all()
    return db_submenus

def create_menu_submenu(db: Session, submenu: SubmenuCreate, menu_id: UUID4):
...
def update_submenu(db: Session, submenu: SubmenuUpdate, submenu_id: UUID4):
...
def delete_submenu(db: Session, submenu_id: UUID4):
    # submenu = get_submenu(db, submenu_id)
    db_submenu = db.query(Submenu).filter(Submenu.id == submenu_id).first()
