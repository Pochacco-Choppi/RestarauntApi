...
async def get_submenu(session: AsyncSession, submenu_id: UUID4):
    stmt = select(
        ...
        Submenu.title,
        Submenu.description, 
        select(func.count(Dish.id)).where(Dish.submenu_id == submenu_id).label("dishes_count"),
        ).where(
...
async def update_submenu(session: AsyncSession, submenu: SubmenuUpdate, submenu_id: UUID4):
