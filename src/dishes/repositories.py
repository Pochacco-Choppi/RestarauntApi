...
class DishRepository:
    ...
    async def get(self, dish_id: UUID4) -> Dish:
        stmt = select(self.model).where(self.model.id == dish_id)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def list(self, submenu_id: UUID4, skip: int = 0, limit: int = 100) -> list[Dish]:
        stmt = (
            select(self.model)
    ...
    async def update(self, dish: DishUpdate, dish_id: UUID4) -> Dish | None:
    ...
    async def create(self, dish: DishCreate, submenu_id: UUID4, **kwargs) -> Dish:
